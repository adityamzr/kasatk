...
export default function AdminShell({ children, user, permissions }: Props) {
  ...
  return <div className="admin-shell">
    ...
    <section className="admin-main"><header className="admin-header"><button className="menu-toggle" aria-label="Buka menu" onClick={() => setOpen(true)}><Menu size={22} aria-hidden="true" /></button><div className="breadcrumb">Dashboard <span>/</span> {pathname === '/admin' ? 'Dashboard' : pathname.startsWith('/admin/spp') ? 'SPP & Tagihan' : pathname.startsWith('/admin/savings/') ? 'Tabungan Siswa / Detail Siswa' : pathname.startsWith('/admin/savings') ? 'Tabungan Siswa' : pathname.startsWith('/admin/students/new') ? 'Data Siswa / Tambah Siswa' : pathname.startsWith('/admin/students/') ? 'Data Siswa / Detail Siswa' : pathname.startsWith('/admin/students') ? 'Data Siswa' : pathname.startsWith('/admin/parents/') ? 'Orang Tua / Detail' : pathname.startsWith('/admin/parents') ? 'Orang Tua / Wali' : pathname.startsWith('/admin/classes') ? 'Kelas' : pathname.startsWith('/admin/users') ? 'Pengguna' : 'Dashboard'}</div><div className="header-actions"><div className="user-chip"><div className="avatar small">{initials}</div><span>{user.name}</span></div></div></header><div className="admin-content">{children}</div></section>
  </div>;
}
