...
import { CircleCheck, GraduationCap, LayoutDashboard, LogOut, Menu, ReceiptText, School, Settings, UserCog, UsersRound, X, type LucideIcon } from 'lucide-react';
...
export default function AdminShell({ children, user, permissions }: Props) {
  ...
  async function logout() { if (loggingOut) return; setLoggingOut(true); await fetch('/api/auth/logout', { method: 'POST' }); router.replace('/login'); router.refresh(); }
  ...
  return <div className="admin-shell">
    ...
    <aside className={open ? 'admin-sidebar open' : 'admin-sidebar'}>
      <div className="brand"><img src="/logo-paud.jpg" alt="Logo PAUD" /><div><b>Cerdas Ceria</b><span>PAUD Finance</span></div><button className="close-drawer" aria-label="Tutup menu" onClick={() => setOpen(false)}><X size={20} aria-hidden="true" /></button></div>
      ...
      <div className="account-area"><div className="profile"><div className="avatar">{initials}</div><div><b>{user.name}</b><span>{user.role}</span></div></div><button className="logout-text" onClick={logout} disabled={loggingOut}><LogOut size={16} strokeWidth={1.8} aria-hidden="true" />{loggingOut ? 'Keluar...' : 'Keluar'}</button></div>
    </aside>
    <section className="admin-main"><header className="admin-header"><button className="menu-toggle" aria-label="Buka menu" onClick={() => setOpen(true)}><Menu size={22} aria-hidden="true" /></button><div className="breadcrumb">Dashboard <span>/</span> {pathname === '/admin' ? 'Dashboard' : pathname.startsWith('/admin/spp') ? 'SPP & Tagihan' : pathname.startsWith('/admin/students/new') ? 'Data Siswa / Tambah Siswa' : pathname.startsWith('/admin/students/') ? 'Data Siswa / Detail Siswa' : pathname.startsWith('/admin/students') ? 'Data Siswa' : pathname.startsWith('/admin/parents/') ? 'Orang Tua / Detail' : pathname.startsWith('/admin/parents') ? 'Orang Tua / Wali' : pathname.startsWith('/admin/classes') ? 'Kelas' : pathname.startsWith('/admin/users') ? 'Pengguna' : 'Dashboard'}</div><div className="header-actions"><div className="user-chip"><div className="avatar small">{initials}</div><span>{user.name}</span></div></div></header><div className="admin-content">{children}</div></section>
  </div>;
}
