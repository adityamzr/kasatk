...
export async function GET() {
  ...
  try {
    ...
    const [activeStudents, payments, openBills] = await prisma.$transaction([
    ...
    return NextResponse.json({ activeStudents, cashInThisMonth: Number(payments._sum.amount ?? 0), totalArrears: arrears });
